...
function OrdersError({ error, onRetry }: Readonly<{ error: string; onRetry: () => void }>) {
  return (
    <Card className="border-muted shadow-none">
      <CardContent className="flex flex-col items-center justify-center gap-4 px-6 py-14 text-center">
...
function OrderCard({ order, currentUserId }: Readonly<{ order: OrderRow; currentUserId: string | null }>) {
  const statusMeta = getOrderStatusMeta(order.status)
  const role = getOrderRoleLabel({ buyerId: order.buyerId, sellerId: order.sellerId }, currentUserId)

  return (
    <Card className="border-muted/50 shadow-sm transition-all duration-300 ease-out hover:shadow-xl hover:-translate-y-1 hover:border-light-green/80 bg-white/50 backdrop-blur-sm group overflow-hidden">
      <CardHeader className="space-y-4 pb-4">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Badge variant="secondary" className="bg-light-green/30 text-olive">
              {role}
            </Badge>
          </div>
        </div>
